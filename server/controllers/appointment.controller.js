...
//     if (isNaN(originalId)) {
//       return res.status(400).json({ error: "Invalid original appointment ID" });
//     }

//     const { newAppointmentId, ...details } = req.body;

//     // Check if newAppointmentId is provided
//     if (!newAppointmentId) {
//       return res.status(400).json({ error: "New appointment ID is required." });
//     }

//     // Validate newAppointmentId
//     if (isNaN(parseInt(newAppointmentId, 10))) {
//       return res.status(400).json({ error: "Invalid new appointment ID" });
//     }

//     // 1. Cancel the original appointment
//     const cancelResult = await updateAppointmentModel(originalId, { status: 'Cancelled' });

//     if (cancelResult.affectedRows === 0) {
//       return res.status(404).json({ error: "Original appointment not found or not cancelled." });
//     }

//     // 2. Transfer details to the new appointment
//     const newAppointmentResult = await updateAppointmentModel(newAppointmentId, {
//       ...details,
//       status: 'Booked',  // Set status to 'Booked' for the new appointment
//     });

//     if (newAppointmentResult.affectedRows === 0) {
//       return res.status(404).json({ error: "New appointment not found or not updated." });
//     }

//     res.status(200).send("Appointment updated and transferred successfully.");
//   } catch (error) {
//     res.status(500).json({ error: error.message });
//   }
// };
...
/*
TODO: Update the updateAppointmentController function to transfer the details of the original appointment to the new appointment.
*/
//update appointment
const updateAppointmentController = async (req, res) => {
  try {
